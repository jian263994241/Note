	...
	$.snowfall = function(element, options){
		var	defaults = {
			...
			function Flake(_x, _y, _size, _speed, _id, _position)
			{
				...
				this.reset = function(){
					if (this.pos == "Left"){
						var _tempx = random(0,(elWidth-900)/2);
					}else{
						var _tempx = random(((elWidth-900)/2+900),elWidth);
					}
			...
			for(i = 0; i < options.flakeCount; i+=1){
				...
				flakes.push(new Flake(_tempx, random(0, elHeight), random((options.minSize * 100), (options.maxSize * 100)) / 100, random(options.minSpeed, options.maxSpeed), flakeId, pos));
			}
